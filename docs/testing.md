...
加入撤销功能后的最终验证（同日）：默认测试 **95 项通过**，模拟黑箱 **2 项通过**；Ruff 检查和格式检查通过。撤销用例覆盖保留回档后的游戏进度、再次撤销、失败回滚、来源变化/缺失、容量限制、权限与确认、服务重启与目录切换中断恢复，以及原始归档已删除时仍可撤销。新增黑箱通过真实插件 Bridge、HTTP MCP 和独立恢复进程验证撤销申请、确认及当前/保留存档内容。测试客户端有 MCP 旧接口弃用提示，不影响通过结果。

三个 Compose 文件仅用 Python YAML 解析器检查语法；同时核对 `.env.example` 全部变量已在配置文档出现、文档相对链接存在、示例启动脚本为 LF。没有运行本地 Docker 或 WSL；这些静态检查不等于 Compose 插值、镜像构建或 NAS 部署验收。

QQ 官方接入修正验证：默认测试 **98 项通过**，Ruff 检查和格式检查通过。新增模拟 `qq_official` 事件，验证非数字群/用户身份、完整字符串权限匹配、私聊拒绝，以及其他群或其他管理员不能代确认；原平台通用测试继续保留。Compose YAML 语法、可选模板仅包含 AstrBot、环境变量说明及本地文档链接检查通过。本次未更改生产 Python 逻辑，未重跑可选黑箱，也未连接真实 QQ 官方平台；前述黑箱结果属于此前恢复与撤销功能验证。

## NAS 上推荐的首次验证顺序

移除恢复客户端 `base_url` 后再次验证：98 项默认测试及 2 项模拟黑箱通过，Ruff 检查与格式检查通过。黑箱的 TCP 替代客户端同步改为完整 RPC URL；此结果仍不替代 Linux Unix socket 的实际部署验证。
...
8. **撤销回档**：根据成功任务编号申请撤销，确认前核对当前世界未变；确认新任务后核实回到原回档前存档，并保留撤销前的世界。需要时再撤销这次撤销，验证可回到撤销前的进度。
