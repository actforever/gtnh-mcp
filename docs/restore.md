...
备份目录默认为 `/gtnh/backups`，位于映射的游戏根目录 `/gtnh` 内，对应 NAS 的 `/volume2/sharev9/minecraft/gtnh/backups`。只枚举目录顶层的普通 `.zip` 和 `.tar.gz` 文件，列表返回 `format` 字段；ID 为文件名 SHA-256，任务另外绑定文件内容 SHA-256。重命名文件会改变 ID，修改文件会使已有恢复申请失效。
...
以下是 QQ 插件操作流程，管理员权限由插件检查。直接 MCP 客户端持固定密钥可申请和调用 confirm_restore，后端要求相同 actor（缺省为 api-client）、十分钟期限和来源指纹一致，不检查 QQ 角色；详见 [权限边界](security.md)。
...
通过 QQ 插件撤销同样需要管理员权限；所有入口都需要相同调用者确认、维护锁和容器退出检查。来源缺失或变化会在停服前失败；撤销后启动失败则尝试回到撤销前世界。任务和来源关系持久化，服务重启后仍可查询、确认或执行中断恢复。重复确认同一撤销编号不会重复执行；对同一个来源重新申请则是另一项需要确认的新操作。
